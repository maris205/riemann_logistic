...
\abstract{Whether a low-dimensional deterministic dynamical system can numerically reproduce features of the non-trivial zeros of the Riemann $\zeta$ function is an open exploratory question. Building on our recent result that the symbolic dynamics of the logistic map at its band-merging point is isomorphic to the prime sieve \cite{wang2026}, we study a non-autonomous quadratic map driven by a logarithmic cooling schedule ($\mu_n \sim 1/\ln^2 n$, introduced here as a phenomenological ansatz rather than a derived law) and construct an empirical, finite-dimensional transfer matrix from its trajectories. This matrix is generally non-normal and dissipative; its complex eigenphases are compared numerically to the Riemann zeros after calibrating a small number of free parameters against the same low-order zeros. We report this comparison as a numerical, in-sample correspondence rather than an independent prediction, and quantify the gap directly: fitting on the first $M\in\{50,70,80\}$ zeros and evaluating on the remaining, unseen zeros gives a held-out mean squared error one to two orders of magnitude larger than the training error, and the fitted coupling itself drifts with $M$; a ten-seed robustness check at fixed $M=70$ shows the fitted coupling is comparatively stable (coefficient of variation $\approx4.7\%$) against the stochastic initial condition. We also discuss the resulting model's behavior at low order ($N\lesssim 20$), where an unselected re-computation and a Spearman/permutation significance test show a qualitative rank correlation ($\rho=0.56$, $p=0.010$) with, but no significant ($p=0.099$) joint co-location of, a residual feature and enlarged error bars reported in recent ion-trap quantum simulations of the same zeros. At larger $N$ ($N\ge 1000$), we compare the model against a globally rescaled Gaussian Unitary Ensemble (GUE) surrogate on the mean counting-function trend (a different comparison from the standard unfolded local-statistics test of random-matrix theory), and we show that restoring the conjugate ($\pm\theta$) eigenphases drives a fitted linear intercept to approximately zero -- a property that, for symmetric regression data, follows in part from the symmetry of the construction itself. A separate check finds that the dynamical model's own eigenphase spacing statistics do not match the GUE prediction under a standard unfolded local-statistics test, unlike the true Riemann zeros. We are explicit throughout that these are numerical observations on a heuristic model, not a proof, a derivation, or a Hilbert--P\'olya-type operator construction, and we tabulate the epistemic status of every main claim.}
...
intrinsic particle--hole symmetry of the eigenphase spectrum affects the
...
\textbf{Topology Maintenance (}\(\text{\ensuremath{\mu}}_{{c}}\)\textbf{):} The
...
dependent on the iteration step \({n}\), forcing the system to
asymptotically approach the critical attractor at the exact macroscopic
rate required by the Riemann zero counting function.
...
scales to this regime without a change of method.
...
\begin{table}[H]
\caption{Performance of macroscopic reconstruction under different phase space domains and boundary constraints. The parameters $k_1$ and $b$ shown here are the result of a global numerical optimization (differential evolution over $k_1 \in [2,15]$, with or without the constraint $b=0$ depending on the strategy) that minimizes the mean squared error against the first 100 Riemann zeros. This optimization protocol differs from the fixed-parameter visualization experiments shown in Figures~\ref{fig4}--\ref{fig5}, which use predetermined values of $k$ to illustrate the qualitative behavior of different anchoring strategies.}\label{tab1}
\small
...
\(\text{\ensuremath{\sim}2.8\%}\). The qualitative behavior of different anchoring
strategies is visually illustrated in {Figure \ref{fig4}} using fixed
predetermined parameters chosen for demonstration purposes.

\begin{figure}[H]
%\centering
\includegraphics[width=\linewidth]{figures/image4.png}
\caption{\textbf{Reconstruction Domains and Anchoring Strategies: A Comparative Analysis of Models A, C, and D.} This figure shows single-run experiments with fixed predetermined values of $k$ (Model A: $k \approx 7.429$, Model C: $k \approx 8.070$, Model D: $k \approx 8.783$) to visually illustrate the qualitative behavior of different anchoring strategies and their residual envelopes over 100 orders. These fixed-parameter experiments differ from the global optimization protocol used in Table~\ref{tab1}, where $k$ is itself the variable being optimized to minimize MSE; the numerical values of $k$ therefore differ between the two experimental setups and should not be directly compared. \textbf{Model A (Orange / Single-Point Anchoring):} Utilizes solely the first zero as the absolute physical baseline. While strictly aligning the ground state, it accumulates the largest macroscopic divergence over 100 orders (MSE \(\text{\ensuremath{\approx}~20.22}\)). \textbf{Model C (Blue / Forced Origin Scaling):} Represents a global least-squares fit constrained to the origin (\({b}\text{~=~0}\)) within the single-sided positive spectrum, which suppresses error divergence relative to Model A, yielding a lower mean squared error (MSE \(\text{\ensuremath{\approx}~10.08}\)). \textbf{Model D (Green / Conjugate Full Spectrum Free Fit):} Uses both positive and negative eigenphases. Because the regression data are symmetric under $(\theta,\gamma)\to(-\theta,-\gamma)$, the least-squares intercept is algebraically forced to \({b}\text{ = 0.000}\) (Section~\ref{sec:conjugate-ablation}); the fitted slope and the resulting residual envelope (MSE \(\text{\ensuremath{\approx}~10.81}\)) are the model-specific numerical content of this comparison.}\label{fig4}
\end{figure}
...
error) once the conjugate branch is included.
...
\textbf{Theoretical Derivation of the Non-Autonomous Scaling Law}\label{sec:scaling-derivation}
...
This derivation rules out empirical forms like
\(\text{1/ln}{n}\), which, due to square-root truncation, would
compress the spectrum too weakly
(\(\text{1/}\sqrt{\text{ln}{n}}\)), causing divergent
...
\(\text{1/}\text{ln}^{\text{2}}{n}\) cooling rate, the
\emph{direction} of approach toward \(\text{U}_{{c}}\) is subject
...
ensuring \(\text{\ensuremath{\mu}}_{{n}}>\text{U}_{{c}}\) at all
times.
...
\item
  ...
  introduce a hard cutoff (\(\text{|}\text{\ensuremath{\lambda}}{|>0.4}\)) and lock
  onto the upper half of the complex plane
  (\(\text{Im(}\text{\ensuremath{\lambda}}{)>0}\)), purifying the true positive-energy
