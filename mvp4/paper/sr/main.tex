...
\begin{abstract}
Finding a dynamical system whose spectrum corresponds to the non-trivial
zeros of the Riemann \(\text{\ensuremath{\zeta}}\) function is a long-standing question in
mathematical physics. Building on our recent result that the symbolic
dynamics of the Logistic map at its band-merging point is isomorphic to
the prime sieve [Wang, \emph{Research in Mathematics} 13(1), 2026], we
study a discrete dynamical operator driven by a non-autonomous
logarithmic cooling schedule
(\(\text{\ensuremath{\mu}}_{\text{n}}\text{\ensuremath{\sim}1/}\text{ln}^{\text{2}}\text{n}\)) and
compare its spectrum numerically to the Riemann zeros. At low and
intermediate orders the model reproduces the first zeros with small
error and exhibits a change of behavior near \(\text{N}\text{\ensuremath{\approx}20}\). We
observe that this feature coincides spatially with the enlarged
measurement deviations reported near the same orders in recent ion-trap
quantum-simulation experiments (e.g., USTC), and we discuss --- as a
hypothesis rather than a demonstrated mechanism --- whether part of that
deviation could reflect a single-sided phase-space truncation rather
than purely instrumental decoherence. Extending the evolution into the
macroscopic regime (\(\text{N}\text{\ensuremath{\ge}1000}\)), an ablation comparison
shows that a globally scaled Gaussian Unitary Ensemble (GUE) surrogate
deviates systematically in this counting-function comparison, consistent
with the fact that a single global scale cannot match a logarithmically
varying mean density; restoring the conjugate (\(\text{\ensuremath{\pm}}\text{\ensuremath{\theta}}\))
spectrum drives the fitted ground-state intercept to approximately zero.
A second-order term motivated by a renormalization-group running
coupling improves the match to the logarithmic densification of the
zeros. We present these results as numerical observations and heuristic
arguments rather than proofs, and we state explicitly which claims are
established, numerical, or conjectural. This offers a non-autonomous,
dissipative-dynamics perspective on the Hilbert--P\'olya program,
complementary to static-Hamiltonian and random-matrix approaches.
\end{abstract}
\begin{document}
...
\[\text{x}_{\text{n}\text{+1}}\text{=1\ensuremath{-}}\text{\ensuremath{\lambda}}_{\text{n}}\text{x}_{\text{n}}^{\text{2}}\]
...
\[\text{\ensuremath{\lambda}}_{\text{n}}\text{=}\text{\ensuremath{\mu}}_{\text{c}}\text{+}\text{\ensuremath{\delta}}_{\text{n}}\]

Here, \(\text{\ensuremath{\mu}}_{\text{c}}\text{\ensuremath{\approx}1.5437}\) is identified as the
...
\[\text{x}_{\text{n}\text{+1}}\text{=1\ensuremath{-}}\underset{\text{Effective}\text{\ensuremath{\lambda}}_{\text{n}}}{\underset{\text{\ensuremath{\underbrace{\phantom{x}}}}}{\left( \text{\ensuremath{\mu}}_{\text{c}}\text{+}\frac{\text{k}_{\text{1}}}{\text{ln}^{\text{2}}\text{n}}\text{+}\frac{\text{k}_{\text{2}}}{\text{ln}^{\text{3}}\text{n}} \right)}}\text{x}_{\text{n}}^{\text{2}}\]
...
low-frequency interval (\(\text{N}\text{=1\ensuremath{\sim}6}\)). Thus far, we have
...
\caption{\textbf{Figure 2. Microscopic empirical validation of the dynamical eigen-spectrum and the emergence of the} \(\text{N=20}\) \textbf{resonance anomaly.} (Top) The alignment trajectory of the dynamical predicted energy levels (\(\text{E}_{\text{pred}}\), colored dots) and the true Riemann zeros (\(\text{\ensuremath{\gamma}}_{\text{n}}\), golden stars) under the critical numerical discrete scale \(\text{\ensuremath{\epsilon}=0.001916}\). In the low-frequency limit (\(\text{N=1}\) to \(\text{6}\), green shaded area), the system achieves high-fidelity ``Quantum Lock-in,'' verifying the accuracy of the ground state linear homomorphic mapping. (Bottom) Actual dynamical residual distribution with signs (\(\text{\ensuremath{\Delta}E=}\text{E}_{\text{pred}}\text{\ensuremath{-}}\text{\ensuremath{\gamma}}_{\text{n}}\)). Despite being constrained by nonlinear state density, causing the system to exhibit natural systematic phase drift at higher energy levels, when evolving to \(\text{N=20}\), the predicted spectrum spontaneously excites an extremely steep, isolated structural breaking---a dynamical anomaly spike (marked in red). This nonlinear resonance anomaly, ``blind-box emerged'' from a purely numerical computing engine under finite precision, achieves a notable homomorphic benchmarking in both position and mutational tendency with the inexplicable massive measurement error bars encountered at the same energy level in recent physical ion-trap quantum simulation experiments.}
\end{figure}
...
\caption{\textbf{Figure 3. Direct Benchmarking: Back-to-back comparison of the dynamical numerical model and physical quantum simulation.} Direct comparison of our numerical prediction residuals (red star line) with USTC physical experimental data (ion-trap quantum simulation under different trap frequencies \(\text{\ensuremath{\Omega}}\)). In the \(\text{N<19}\) region, the numerical model successfully captured the near-zero fluctuation baseline of the physical system. Crucially, at \(\text{N=19\ensuremath{\sim}20}\), the numerical computation's ``anomaly spike'' and the severe divergence of experimental measurement uncertainty (red shaded area) completely align in position and qualitative characteristics. This high degree of commonality provides strong evidence that ``experimental collapse is an intrinsic topological property of finite-precision dynamics rather than random instrumental error.''}
\end{figure}
...
numerical \(\text{\ensuremath{\epsilon}}\)).
...
the feature mutation position (\(\text{N}\text{\ensuremath{\approx}20}\)).
...
a resolution of \(\text{\ensuremath{\sim}}\text{10}^{\text{\ensuremath{-}2}}\)). However, those
...
\(\text{N}\text{=19\ensuremath{\sim}20}\)) violate the conventional Gaussian noise
...
system (or instrument) is at the \(\text{10}^{\text{\ensuremath{-}2}}\) to
\(\text{10}^{\text{\ensuremath{-}3}}\) scale limit, both encountered an ``information
...
\(\text{\ensuremath{\sim}4.0\%/>8.0\%}\)\tabularnewline
...
& \(\text{\ensuremath{\sim}1.4\%/\ensuremath{\sim}2.8\%}\)\tabularnewline
...
\(\text{\ensuremath{\sim}1.5\%/\ensuremath{\sim}1.5\%}\)\tabularnewline
\bottomrule
\end{longtable}
...
\caption{\textbf{Figure 5: Spectral Showdown: Hardware Limitations vs.~Dynamics Model Topology.}  * \textbf{Scatter Points and Purple Shading (Quantum Hardware):} Displays the actual measurement deviations from the University of Science and Technology of China (USTC) ion-trap quantum hardware. Primarily limited by quantum decoherence effects, the experimental hardware experiences signal breakdown in the \(\text{N}\text{\ensuremath{\approx}76\ensuremath{-}80}\) band, establishing a physical error envelope restricted to \(\text{\ensuremath{\pm}1.96\%}\).\textbf{Red Solid Line (Dynamics Model):} Represents the theoretical deviation trajectory derived from 10 billion steps of evolution using the proposed dynamics model (Model C: Single-sided positive spectrum forced origin scaling, \(\text{k}_{\text{1}}\text{=8.070}\)). The model's mean error is 2.69\%. The model successfully overcomes the hardware's physical limitation boundary, extending deterministic predictions stably to the \(\text{N}\text{=100}\) order. Notable is the surprising spatial synchronization between the macroscopic oscillation peaks of the theoretical model (red line) and the local error extrema of the quantum hardware (around \(\text{N}\text{\ensuremath{\approx}20,40,76}\)).}
\end{figure}
...
\(\text{N}\text{\ensuremath{\approx}20,40}\)). Our dynamical model reveals these intense
spikes as un-broken conjugate tearing events. This robustly confirms
that the experimental divergences and the \(\text{\ensuremath{\sim}1.96\%}\) empirical
...
drop (from \(\text{6.48\ensuremath{\rightarrow}4.39}\)). Most crucially, under completely
...
\(\text{100}\)-zero regime to \(\text{\ensuremath{\sim}12.78}\) in the deep-water
...
In the shallow-water regime (\(\text{N}\text{\ensuremath{\le}100}\) at \(\text{2,000}\)
...
\caption{\textbf{Figure 7: Ultra-Macroscopic Spectral Alignment (}\(\text{N}\text{=10,000}\)\textbf{): Thermodynamic Cooling vs. Statistical Baselines.} M-GUE (Purple Dashed / Gaussian Unitary Ensemble): The pure Random Matrix Theory baseline suffers a catastrophic structural divergence (MSE \(\text{\ensuremath{\approx}255455.9}\)). This visually exposes that without a deterministic cooling mechanism, Wigner's Semicircle Law fundamentally fails to capture the macroscopic logarithmic density of the Riemann manifold.M1 (Orange Solid / 1D Thermodynamic Cooling): The 1D non-autonomous dynamical model (\(\text{k}_{\text{1}}\text{\ensuremath{\approx}13.431}\)) effectively paces the true Riemann zeros. It successfully anchors both the highly sensitive low-frequency ground states and the deep-water high-frequency states.}
\end{figure}
...
\(\text{N}\text{\ensuremath{\ge}1000}\) deep-water regime, the system required an
...
current \(\text{\ensuremath{\sim}1.96\%}\) dispersion envelope boundary, they must
...
isomorphism, we must enforce \(\text{\ensuremath{\Delta}\ensuremath{\Phi}\ensuremath{\propto}\ensuremath{\Delta}}\text{E}\). Equating the
constraints yields \(\sqrt{\text{\ensuremath{\Delta}}\text{\ensuremath{\mu}}}\text{\ensuremath{\propto}1/ln}\text{n}\),
